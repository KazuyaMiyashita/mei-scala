...
import generator.{MeiModelWriterWriter, RelaxNGParser}

class MeiModelWriterWriterSpec extends AnyFunSuite {

  test("Generate writer code from mei-zero.rng") {
    runEach("/mei-zero.rng", "mei.zero", "/zero/writer/MeiModelWriter.scala")
  }
  test("Generate writer code from mei-minimum.rng") {
    runEach("/mei-minimum.rng", "mei.minimum", "/minimum/writer/MeiModelWriter.scala")
  }
  test("Generate writer code from mei-basic.rng") {
    runEach("/mei-basic.rng", "mei.basic", "/basic/writer/MeiModelWriter.scala")
  }
  test("Generate writer code from mei-cmn.rng") {
    runEach("/mei-cmn.rng", "mei.cmn", "/cmn/writer/MeiModelWriter.scala")
  }
  ...
  private def runEach(filename: String, packageName: String, expectedFile: String): Unit = {
    ...
    val generatedCode       = MeiModelWriterWriter.generate(semanticDefinitions, packageName)
